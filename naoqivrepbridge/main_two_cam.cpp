...
bool get_Nao_Handles(int clientID) {
	...
	all_ok &= (simxGetObjectHandle(clientID, "NAO_vision2", 		&_nao_bottom_camera_handle, simx_opmode_oneshot_wait) == simx_return_ok);
	...
	if(all_ok) {
		...
		simxGetVisionSensorImage(clientID, _nao_bottom_camera_handle, _camera_resolution, image, 0, simx_opmode_streaming); // Start streaming operation
	}
...
int main(int argc, char *argv[])
{
	...
	string subscriberID = "vrep_bridge_top";
	int fps = 10;
	int camera_period = 1000/fps;
	// The subscriberID can be altered if other instances are already running
	string camIDTop = videoProxy.subscribeCamera(subscriberID, AL::kTopCamera, AL::kVGA, AL::kRGBColorSpace, fps);

	subscriberID = "vrep_bridge_bot";
	string camIDBot = videoProxy.subscribeCamera(subscriberID, AL::kBottomCamera, AL::kVGA, AL::kRGBColorSpace, fps);
		...
		if(start_time > next_camera_refresh) {
			if(not camera_update.joinable()) {
				camera_update = thread([clientID, &videoProxy, &image_tab](){
					...
					simxGetVisionSensorImage(clientID, _nao_bottom_camera_handle, _camera_resolution, &image, 0, simx_opmode_buffer);

					for (int y = 0, idx = 0; y < 480; ++y)
						for (int x = 0; x < line_size; ++x, ++idx)
							image_tab[_image_size - (y+1)*line_size + x] = image[idx];

					videoProxy.putImage(AL::kBottomCamera, _camera_resolution[0], _camera_resolution[1], image_tab);
				});
	...
	videoProxy.unsubscribe(camIDTop);
	videoProxy.unsubscribe(camIDBot);
